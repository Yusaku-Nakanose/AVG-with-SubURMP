...
class SpectramDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        ### Load CLAP features
        # /path/to/spec/*.png → /path/to/clap_features/*.npy
        clap_path = audio_path.replace('/spec/', '/clap_features/').replace('.png', '.npy')
        try:
            clap_features = np.load(clap_path)
            clap_features = torch.from_numpy(clap_features).float()
        except Exception as e:
            print(f"Error loading CLAP features from {clap_path}: {e}")
            # フォールバック: 512次元のゼロベクトル
            clap_features = torch.zeros(1, 512).float()

        input_dict = {
            'audio': audio_tensor, 
            'image1': image_tensor1, 
            'image2': image_tensor2, 
            'image3': image_tensor3, 
            'image4': image_tensor4, 
            'path': audio_path,
            'clap_features': clap_features  # CLAP特徴量を追加
        }
